...
Since the road pilot, this dataset is queried with **real** Oregon road geometry (U.S. Census Bureau TIGER/Line 2025 centerlines); the ecoregion code path is unchanged. See [docs/ROADS.md](ROADS.md) for the road source, composition rules, and measured overlap results, and [docs/ARCHITECTURE.md](ARCHITECTURE.md) for how per-dataset coverage is reported.
