...
let origin = null;
try { origin = git('rev-parse', `origin/${BRANCH}`); } catch { origin = null; }
if (origin && origin !== head) throw new Error(`HEAD (${head.slice(0, 7)}) is not what is on origin/${BRANCH} (${origin.slice(0, 7)}): push first, then deploy`);
